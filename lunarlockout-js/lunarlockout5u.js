...
var mousex, mousey, mousePicx, mousePicy;
border();
...
document.onmousedown =function(e)
{
  ...
  mousePicx=Math.floor((mousex+35)/38);
  mousePicy=Math.floor((mousey-24)/38);
  if (mousePicx<6 && mousePicy<6) mousetest();
}

function mousetest()
{
  var xa, ya;
  var v=map[mousePicx+7*mousePicy];
  if (v>0)
  {
    player=v;
  }
  ...
  {
    if (mousePicx>px[player]) xa=1; else xa=0;
//alert(" "+mousePicx+px[player]);
    if (mousePicx<px[player]) xa=-1;
    if (mousePicy>py[player]) ya=1; else ya=0;
    if (mousePicy<py[player]) ya=-1;
    if (xa*ya == 0 && xa+ya != 0) move(xa,ya);
  }
}
